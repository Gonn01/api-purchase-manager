...
export async function createPurchase(ignored, image, amount, amount_per_quota, number_of_quotas, payed_quotas, currency_type, name, type, financialEntityId, fixedExpense) {
  try {
    ...
    const query = `
      ...
      RETURNING *
    `;
    const result = await executeQuery(query, [ignored, image, amount, amount_per_quota, number_of_quotas, payed_quotas, currency_type, name, type, financialEntityId, fixedExpense]);
